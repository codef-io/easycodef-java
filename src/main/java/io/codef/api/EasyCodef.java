...
/**
 * Codef 호출을 위한 간편 연동 오픈소스 라이브러리 클래스
 */
public class EasyCodef {
    ...
    private boolean checkClientInfo(int serviceType) {
        if (serviceType == 0) {
            ...
            return properties.getClientSecret() != null && !"".equals(properties.getClientSecret().trim());
        } else if (serviceType == 1) {
            if (properties.getDemoClientId() == null || "".equals(properties.getDemoClientId().trim())) {
                return false;
            }
            return properties.getDemoClientSecret() != null && !"".equals(properties.getDemoClientSecret().trim());
        } else {
            if (EasyCodefConstant.SANDBOX_CLIENT_ID == null || "".equals(EasyCodefConstant.SANDBOX_CLIENT_ID.trim())) {
                return false;
            }
            return EasyCodefConstant.SANDBOX_CLIENT_SECRET != null && !"".equals(EasyCodefConstant.SANDBOX_CLIENT_SECRET.trim());
        }
    }
    ...
    private boolean checkPublicKey() {
        return properties.getPublicKey() != null && !"".equals(properties.getPublicKey().trim());
    }
    ...
    /**
     * 토큰 반환 요청 - 보유 중인 유효한 토큰이 있는 경우 반환, 없는 경우 신규 발급 후 반환
     */
    public String requestToken(EasyCodefServiceType serviceType) throws IOException {
    ...
    /**
     * 토큰 신규 발급 후 반환(코드에프 이용 중 추가 업무 사용을 하는 등 토큰 권한 변경이 필요하거나 신규 토큰이 필요한 경우시 사용)
     */
    public String requestNewToken(EasyCodefServiceType serviceType) throws IOException {
        String clientId = null;
        String clientSecret = null;
